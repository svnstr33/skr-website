...
export function StandardPage({ content }: { content: PageContent }) {
  ...
  return <main className="detail-page" aria-labelledby="detail-page-title">
    ...
    {isProducts && <section className="content-section product-explorer"><div className="section-title explorer-title"><div><p className="eyebrow">Product Explorer</p><h2>Find the right product.</h2></div><span><FiSliders /> Filter by division</span></div><div className="explorer-controls"><label><FiSearch /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search product or application" /></label><div>{(['All', 'Umbrella', 'Engineering'] as const).map((item) => <button className={filter === item ? 'active' : ''} onClick={() => setFilter(item)} key={item}>{item}</button>)}</div></div><div className="product-grid">{visibleProducts.map(([code, name, description, group]) => <article key={code}><a className="product-card-link" href={`#product-${code.toLowerCase()}`}><div className={`product-thumb ${group.toLowerCase()}`}><span>{group === 'Umbrella' ? 'Umbrella Division' : 'Engineering & Hardware'}</span></div><small>{code}</small><h3>{name}</h3><p>{description}</p><em>View product details <FiArrowUpRight /></em></a></article>)}{!visibleProducts.length && <p className="empty-state">No matching product found. Please send us your drawing or requirement.</p>}</div></section>}
