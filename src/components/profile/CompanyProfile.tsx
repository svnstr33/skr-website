...
export function CompanyProfile({ visible }: CompanyProfileProps) {
  return <section id="profile" className={`company-profile about-rich ${visible ? 'profile-visible' : 'profile-hidden'}`} aria-labelledby="company-profile-title">
    <div className="profile-heading"><div><p className="eyebrow">About SKR Metal Industries Pvt. Ltd.</p><p className="breadcrumb">Home <span>/</span> About SKR</p><h1 id="company-profile-title">Precision manufacturing for dependable industrial performance.</h1></div><div className="company-status"><span className="status-dot" /> Active</div></div>
    ...
    <div className="directors-card"><p className="card-kicker">Leadership</p><h3>Director Details</h3><div className="directors-grid">{directors.map(([din, name, role]) => <article key={din} className="director-profile-card"><div className="director-photo-placeholder" role="img" aria-label={`${name} photo placeholder`}><span>{name.split(' ').map((part) => part[0]).join('')}</span><small>Photo</small></div><div className="director-profile-content"><h4>{name}</h4><p>{role}</p><dl><div><dt>DIN</dt><dd>{din}</dd></div></dl></div></article>)}</div></div>
  </section>
}
