...
export function CompanyProfile({ visible }: CompanyProfileProps) {
  return <section id="profile" className={`company-profile about-rich ${visible ? 'profile-visible' : 'profile-hidden'}`} aria-labelledby="company-profile-title">
    ...
    <div className="directors-card"><p className="card-kicker">Leadership</p><h3>Director Details</h3><div className="directors-grid">{directors.map(([din, name, role, date]) => <article key={din} className="director-profile-card"><div className="director-photo-placeholder" role="img" aria-label={`${name} photo placeholder`}><span>{name.split(' ').map((part) => part[0]).join('')}</span><small>Photo</small></div><div className="director-profile-content"><h4>{name}</h4><p>{role}</p><dl><div><dt>DIN</dt><dd>{din}</dd></div><div><dt>Begin Date</dt><dd>{date}</dd></div></dl></div></article>)}</div></div>
  </section>
}
